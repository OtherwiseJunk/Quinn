...
watch(selectedGuild, (guildId) => {
  selectedUser.value = "";
  store.memories = [];
  if (guildId) store.fetchUsers(guildId);
});

watch(selectedUser, (userId) => {
  if (userId && selectedGuild.value) {
    store.fetchMemories(selectedGuild.value, userId);
  }
});

async function addMemory() {
  if (!selectedGuild.value || !selectedUser.value || !newContent.value.trim()) return;
  adding.value = true;
  try {
    await store.create(selectedGuild.value, selectedUser.value, newContent.value.trim());
    newContent.value = "";
  } finally {
    adding.value = false;
  }
...
async function saveEdit(memoryId: number) {
  ...
  await store.update(selectedGuild.value, memoryId, editContent.value.trim(), selectedUser.value);
  cancelEdit();
}
...
async function confirmDelete() {
  ...
  deleting.value = true;
  try {
    await store.remove(selectedGuild.value, deletingId.value, selectedUser.value);
  } finally {
...
<template>
  <AppShell>
    <h1 class="page-title">User Memories</h1>
    <p class="page-desc">Quinn's memories about specific users per server.</p>
    ...
    <div class="filters">
      ...
      <div v-if="selectedGuild" class="field">
        <label class="field__label">User</label>
        <select v-model="selectedUser" class="field__select">
          <option value="" disabled>Select a user</option>
          <option v-for="u in store.users" :key="u.userId" :value="u.userId">
            {{ u.displayName }} ({{ u.count }})
          </option>
        </select>
        <EmptyState v-if="store.usersStatus === 'idle' && store.users.length === 0 && selectedGuild" message="No users have memories in this server." />
      </div>
    </div>

    <template v-if="selectedGuild && selectedUser">
      ...
      <EmptyState v-else-if="store.memories.length === 0" message="No memories for this user yet." />
